(() => {
  ...
  const GRID_NATIVE = { x: 420, y: 238, w: 1080, h: 612 };
  ...
  function syncBoardGeometry(){
    ...
    // Align hitboxes exactly over the baked 6x6 grass board whenever the screen is landscape.
    if(vw / vh >= 1.10){
  ...
  function renderBoard(spawned=[],matching=[]){
    ...
    board.forEach((tile,i)=>{
      ...
      // Busy state is enforced by event guards. Do not permanently disable
      // freshly rendered cells while an animation is finishing.
      cell.disabled=phase==='wave'||phase==='gameover';
      if(tile.kind!=='empty'){
        // The stage is the safe area of one grass square. The sprite keeps its
        // natural aspect ratio and can never spill into the neighbouring tile.
        const stage=document.createElement('span');
        stage.className='asset-stage';
        const img=document.createElement('img');
        img.className=`asset asset-${tile.kind}-${tile.type}`;
  ...
  workshopBtn.addEventListener('click',async()=>{
    ...
    // During the workshop we intentionally do NOT spawn new materials.
    // When the workshop ends we must refill every empty slot; otherwise,
    // after the day-10 boss, the board progressively remains empty.
    const spawned=collapse(true);
    updateHUD();
    renderBoard(spawned);
    await wait(180);
    ...
    renderBoard();
    updateHUD();
    showToast('OFFICINA TERMINATA · NUOVI MATERIALI · 5 MOSSE');
  });
